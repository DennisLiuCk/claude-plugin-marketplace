...
11. 對於步驟 8 中的每個發現，啟動一個並行 **Haiku 代理**進行評分。
...
12. 等待所有評分代理完成，收集所有評分結果

13. 將評分結果寫入：`.legacy-analysis/session-{timestamp}/03-scores.json`
...
20. 將結構化資料寫入：`.legacy-analysis/session-{timestamp}/04-findings-structured.json`

## 階段 5: 教學文件生成

21. 使用 **Sonnet 代理**生成教學文件。
...
22. 將生成的文件寫入：`.legacy-analysis/session-{timestamp}/05-PROJECT-ANALYSIS.md`
...
- **並行執行代理**：
  - 階段 2 的 6 個分析代理應並行啟動（在單個回應中使用多個 Task 工具）
  - 階段 3 的 N 個評分代理應並行啟動
  - 不要等待單個代理完成才啟動下一個
...
- **成本優化**：
  - 資格檢查和快速掃描使用 Haiku（快速、便宜）
  - 深度分析使用 Sonnet（推理能力強）
  - 評分使用 Haiku（任務簡單、數量多）
  - 文件生成使用 Sonnet（需要創造力和結構化能力）

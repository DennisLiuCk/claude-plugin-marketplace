...
**特色**：
- 從使用者查詢推導搜尋關鍵字（中英文、方法名、類別名）
- 深度優先追蹤調用鏈（Controller → Service → Repository → Entity）
- 支援 MyBatis Mapper XML 分析
...
- `analyze-java-domain` 命令重大改進 - 精簡報告策略：
  - **文件目的聚焦**：專注於「幫助理解現有專案」，刪除所有改善建議
  - **行數大幅縮減**：從 ~4000 行縮減至 ~850 行（減少約 80%）
  - **刪除冗餘內容**：
    - 效能優化建議
    - 測試代碼範例
    - 假設性/範例性程式碼
  ...
  - **程式碼引用原則**：只保留 5-15 行關鍵邏輯，引導讀者看原始檔案
  - **各文件目標行數**：
    - 00-INDEX.md: ~50 行
    - 01-overview.md: ~150 行
    - 02-call-chains.md: ~200 行
    - 03-data-model.md: ~200 行
    - 04-business-rules.md: ~150 行
    - 05-guide.md: ~100 行
  - **禁用詞彙**：「建議」「應該」「優化」「未來」

**v1.2.0** (2025-11-25)
- `analyze-java-domain` 命令改進：
  - 採用分檔策略生成 6 個獨立文件，避免 token 限制問題
  - 新增索引頁（00-INDEX.md）提供快速導覽
  - 並行生成文件提高效率
  - 時間估算調整為 4-6 分鐘

**v1.1.0** (2025-11-25)
- 新增 `analyze-java-domain` 命令：支援特定領域深度分析
  - 從中文領域關鍵字推導搜尋策略
  - 深度優先追蹤調用鏈
  - 支援 MyBatis Mapper XML 分析
  - 限制搜尋範圍為 `.java` 和 `.xml`（排除 pom.xml）
- 強化 `analyze-java` 命令的評分階段強制性檢查

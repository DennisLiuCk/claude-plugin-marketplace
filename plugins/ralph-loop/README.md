...
Ralph Loop 是一種基於連續 AI 代理循環的開發方法。正如 Geoffrey Huntley 所描述的：**「Ralph 是一個 Bash 循環」** - 一個簡單的 `while true`，反覆向 AI 代理提供提示檔案，允許它迭代改進其工作直到完成。
...
- 提示在迭代之間從不改變
- Claude 之前的工作保存在檔案中
- 每次迭代都看到修改後的檔案和 git 歷史記錄
- Claude 通過讀取檔案中自己過去的工作自主改進
...
**選項：**
- `--max-iterations <n>` - N 次迭代後停止（預設：無限制）
...
**注意**：`--completion-promise` 使用精確字串匹配，因此您不能將其用於多個完成條件（如「SUCCESS」與「BLOCKED」）。始終依賴 `--max-iterations` 作為您的主要安全機制。
...
### 2. 失敗是數據
「確定性地差」意味著失敗是可預測且有資訊量的。使用它們來調整提示。
...
- 在 Y Combinator 黑客馬拉松測試中成功在一夜之間生成了 6 個儲存庫
- 以 297 美元的 API 成本完成了一份 5 萬美元的合同
- 使用這種方法在 3 個月內創建了整個程式語言（「cursed」）
...
│   └── plugin.json      # 插件元資料
├── commands/
│   ├── ralph-loop.md    # 啟動循環命令
│   └── cancel-ralph.md  # 取消循環命令
├── hooks/
│   ├── hooks.json       # 鉤子配置
│   └── stop-hook.sh     # Stop 鉤子腳本
├── scripts/
│   └── setup-ralph-loop.sh  # 設置腳本
└── README.md            # 本文件
```

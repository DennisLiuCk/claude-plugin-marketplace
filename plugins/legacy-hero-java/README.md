...
為了避免 subagent 分析結果截斷或丟失，**Legacy Hero Java 1.1.0** 引入了完整的檔案輸出機制：
...
└── session-{timestamp}/
    ...
    ├── 05-documentation-final.md       # 最終審核通過文件
    └── session-manifest.json           # 會話清單
```

#### 優勢
- ✅ **防止輸出截斷**：每個 agent 的完整分析都保存在獨立檔案中
- ✅ **可追溯性**：所有中間結果都保留，便於後續參考
- ✅ **可恢復性**：即使分析中斷，也可以從檔案恢復
- ✅ **版本控制**：分析結果可以提交到 Git，作為專案文件的一部分
...
├── src/
├── pom.xml
└── .legacy-hero-workspace/           # ← 自動創建
    ├── session-20250123-143022/      # 第一次分析
    │   ├── 01-project-scan.md
    ...
    │
    └── session-20250123-150315/      # 第二次分析
        └── ...
```

**重要提示**：
- `.legacy-hero-workspace/` 目錄可以加入 `.gitignore`（如果不想提交分析結果）
- 或者提交到 Git，作為專案文件的一部分（推薦）
- 最終文件 `05-documentation-final.md` 也會同時保存在 `docs/` 目錄
...
- URL: POST /api/orders
- ✨ 創建工作目錄：.legacy-hero-workspace/session-20250123-143022/
...
- 檢查權限控制
- 追蹤資料庫操作
- ✨ 完整分析已寫入：02-endpoint-analysis-create-order.md
...
- 解釋 @Transactional
- 說明 DTO 模式
- ✨ 知識補充已寫入：03-knowledge-enrichment.md
...
- ✨ 文件草稿已寫入：04-documentation-draft.md
...
- 確認技術陳述準確性：通過
- 審核結果：通過
- ✨ 最終文件已寫入：05-documentation-final.md

階段七：交付報告
✨ 分析完成！所有結果保存在：
  工作目錄：.legacy-hero-workspace/session-20250123-143022/

  可用檔案：
  - 01-project-scan.md (專案概覽)
  - 02-endpoint-analysis-create-order.md (完整端點分析)
  - 03-knowledge-enrichment.md (技術知識補充)
  - 04-documentation-draft.md (文件草稿)
  - 05-documentation-final.md (最終審核通過文件) ← 推薦閱讀
  - session-manifest.json (會話清單)

  同時保存在：docs/api-create-order-analysis.md
```
